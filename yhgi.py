...
def module_with_icons(title, table, fields, display_cols, search_cols):
    ...
    if search:
        ...
        params = [f"%{search}%" for _ in search_cols]
        df = pd.read_sql_query(f"SELECT * FROM {table} WHERE {conditions}", conn, params=params)
    else:
        ...
        if not df.empty:
            ...
            with col2:
                if st.button("🗑️ Delete Record", use_container_width=True, key=f"delete_{table}"):
                    if st.checkbox("Confirm delete", key=f"confirm_{table}"):
